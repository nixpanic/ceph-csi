...
func dataToMap(data string) (map[string]interface{}, error) {
	ms := make(map[string]string)
	err := json.Unmarshal([]byte(data), &ms)
	if err != nil {
		err = fmt.Errorf("failed to convert json to map: %w", err)
	}
...
// kmsConfigHeader is used to detect the type of configuration and provider
// that should be used to parse the configuration from the ConfigMap.
type kmsConfigHeader struct {
	// KmsPROVIDER is generated by the OCS UI and is a signal that the
	// configuration is conforming to the standardVault struct
	KmsPROVIDER string `json:"KMS_PROVIDER"`

	// EncryptionKMSType indicates that the configuration is manually
	// entered and all options should be converted from json to a
	// map[string]string.
	EncryptionKMSType string `json:"encryptionKMSType"`
}

func kmsConfigHeaderNew(data string) (*kmsConfigHeader, error) {
	ch := kmsConfigHeader{}
...
// isVaultTokensConfigMap returns true when the configuration is in the
// standardVault format and needs converting to vaultTokenConf.
func (kch *kmsConfigHeader) isVaultTokensConfigMap() bool {
	return kch.KmsPROVIDER == kmsTypeVaultTokens
}
...
func getVaultConfiguration(namespace, name string) (map[string]interface{}, error) {
	...
	for k, v := range cm.Data {
		// each k is a KMS configuration, with v containing the options
		// in json format

		// inspect the type of configuration
		ch, err := kmsConfigHeaderNew(v)
		if err != nil {
			return nil, fmt.Errorf("failed to detect provider type for %q: %w", k, err)
		}
		if ch.isVaultTokensConfigMap() {
			sv, err := dataToStandardVault(k, v)
			if err != nil {
				return nil, fmt.Errorf("could not parse Vault config for %q: %w", k, err)
			}
			config[k] = sv
		} else {
			options, err := dataToMap(v)
			if err != nil {
				return nil, fmt.Errorf("could not parse options for %q: %w", k, err)
			}
			config[k] = options
		}
	}
