...
func (cs *Server) CreateVolume(
	...
	// smb does not support shallow snapshots
	...
	smbVolume, err := smb.NewSMBVolume(ctx, backend.GetVolumeId())
	if err != nil {
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
	...
	err = smbVolume.CreateShare(backend)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "failed to create SMB share: %v", err)
	}

	log.DebugLog(ctx, "published SMB-share: %s", smbVolume)

	// volume has been shared over SMB, set the "share" parameter to
	// allow mounting
	backend.VolumeContext["share"] = smbVolume.GetSharePath()
...
func (cs *Server) DeleteVolume(
	...
	smbVolume, err := smb.NewSMBVolume(ctx, volumeID)
	if err != nil {
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
	...
	err = smbVolume.DeleteShare()
	// if the share does not exist, continue with deleting the backend volume
	if err != nil && !errors.Is(err, smb.ErrNotFound) {
		return nil, status.Errorf(codes.InvalidArgument, "failed to delete SMB share: %v", err)
	}

	log.DebugLog(ctx, "SMB-share %q has been deleted", smbVolume)

	return cs.backendServer.DeleteVolume(ctx, req)
}

// ControllerExpandVolume calls the backend (CephFS) procedure to expand the
// volume. There is no interaction with the SMB-server needed to publish the
// new size.
...
// There is no interaction with the SMB-server needed for snapshot creation.
...
// There is no interaction with the SMB-server needed for snapshot deletion.
...
// the [SMBVolume] object (which stores the parameters in the volumes OMAP).
func (cs *Server) ControllerModifyVolume(
	...
	if servername, ok := req.GetMutableParameters()[smb.ParameterServer]; ok {
		err := smbVolume.SetServer(servername)
		if err != nil {
