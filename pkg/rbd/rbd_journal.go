...
func (rv *rbdVolume) Exists(ctx context.Context) (bool, error) {
	...
	imageData, err := volJournal.CheckReservation(ctx, rv.Monitors, rv.Creds, rv.JournalPool,
	...
	err = updateVolWithImageInfo(ctx, rv, rv.Creds)
	if err != nil {
		if _, ok := err.(ErrImageNotFound); ok {
			err = volJournal.UndoReservation(ctx, rv.Monitors, rv.Creds, rv.JournalPool, rv.Pool,
	...
	rv.VolID, err = util.GenerateVolID(ctx, rv.Monitors, rv.Creds, imageData.ImagePoolID, rv.Pool,
