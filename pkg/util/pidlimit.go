...
// 7:pids:/kubepods.slice/kubepods-besteffort.slice/kubepods-besteffort-pod82482a84_aaaa_11e9_8f2e_00e919049bad.slice/crio-a35eded02dc141f6e00f0166d5b27345c3fdbd43e96c56a85eb15a2a71fb71e0.scope
...
func GetAndSetPIDLimit(limit int) error {
	currentLimit, err := GetPIDLimit()
	if err != nil {
		return fmt.Errorf("Failed to get the PID limit, can not reconfigure: %v", err)
	} else {
		klog.Infof("Initial PID limit is set to %d", currentLimit)
		err = SetPIDLimit(limit)
		if err != nil {
			return fmt.Errorf("Failed to set new PID limit to %d: %v", limit, err)
		} else {
			s := ""
			if limit == -1 {
				s = " (max)"
			}
			klog.Infof("Reconfigured PID limit to %d%s", limit, s)
		}
	}
