...
func createNVMeoFStorageClass(
	...
	sc.Parameters["csi.storage.k8s.io/provisioner-secret-name"] = "rook-csi-rbd-provisioner" //rbdProvisionerSecretName

	sc.Parameters["csi.storage.k8s.io/controller-publish-secret-namespace"] = cephCSINamespace
	sc.Parameters["csi.storage.k8s.io/controller-publish-secret-name"] = "rook-csi-rbd-provisioner" //rbdProvisionerSecretName

	sc.Parameters["csi.storage.k8s.io/node-stage-secret-namespace"] = cephCSINamespace
	sc.Parameters["csi.storage.k8s.io/node-stage-secret-name"] = "rook-csi-rbd-provisioner" // rbdNodePluginSecretName
	...
	framework.Logf("configuring StorageClass for gatway %q at %s", gwHost, gwIP)
